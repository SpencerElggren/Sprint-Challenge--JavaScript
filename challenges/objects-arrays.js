...
const unisWithUni = [];
function uniUniversities (graduates) {
  for (i=0;i<graduates.length;i++) {
...
function getNames(animals) {
  animals.forEach((name) => displayNames.push(`Name: ${name.animal_name} Scientific: ${name.scientific_name}.`))
}
getNames(zooAnimals);
...
function lowCase(names) {
  return names.map((item) => item.animal_name.toLowerCase());
}
const lowCaseAnimalNames = lowCase(zooAnimals);
...
*/
const popIndividual = zooAnimals.map((item) => item.population);
const populationTotal = popIndividual.reduce((a,b) => a + b);
